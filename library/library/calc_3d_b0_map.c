...
//cc -fPIC -shared -o calc_3d_b0_map.so calc_3d_b0_map.c

/*
This code implements the maximum asymmetry WASSR B0map algorithm
function [freqWMap] = wassr_b0map3d(ppmlist, posimage, negimage, mask)
 */
#ifdef SKIP
...
double * calc_3d_b0_map(double *ppmlist, double *pimg, double *nimg, long *mask,
                        int nppm, int nelemimg, int nelem)

{
    /*Inside mexFunction---*/
    /*Declarations*/
    // double  *ppmlistasc; /* Pointers to ppmlist,W,H,pos and negative images, and mask arrays */
//    mwSize intfactor, ielem, ippm1, ippm2; //ndim, ndimimg, nppm, nelem, nelemimg
    const mwSize *dim_array_mask; //, *dim_array_image;
//    mwSize i1, i2,nzsp1, nzsp2, rangezsp2, minzsp2index, minSSEindex; //, W, H, D;
    // mwSize *psort;
    double maxppm, minppm, stepppm, stepppm2, ftemp1, ftemp2, maxpimg, maxnimg;
    // double *zspp1, *zspn1, *zsp1, *zsp1r, *xzsp, *xzsp2, *zsp2r, *y2;
    double cfcoarse; // *B0map,
    double b0ppmstep;
    double temp1,temp2;
...
//    double temp1,temp2;
    ...
    // ppmlist = mxGetPr(prhs[0]);
    // nppm=mxGetNumberOfElements(prhs[0]);
    // pimg = mxGetPr(prhs[1]);
    // nimg = mxGetPr(prhs[2]);
    // nelemimg = mxGetNumberOfElements(prhs[2]);
    // ndimimg = mxGetNumberOfDimensions(prhs[2]);
    // dim_array_image=mxGetDimensions(prhs[2]);
    // mexPrintf("Himg %i Wimg %i Dimg %i Zimg %i\n",
               // dim_array_image[0],dim_array_image[1],dim_array_image[2],dim_array_image[3]);

    // mask = mxGetPr(prhs[3]);
    /* Get the dimensions in the mask array */
    // nelem = mxGetNumberOfElements(prhs[3]);
    // ndim=mxGetNumberOfDimensions(prhs[3]);
    // ndim=mxGetNumberOfDimensions(prhs[3]);
    // dim_array_mask=mxGetDimensions(prhs[3]);
    // H = dim_array_mask[0];
    // W = dim_array_mask[1];
    // D = dim_array_mask[2];
    // dim_array_mask = mxGetDimensions(mask);
    // mwSize H = dim_array_mask[0];
    // mwSize W = dim_array_mask[1];
    // mwSize D = dim_array_mask[2];
    // printf("H %i W %i D %i\n",H,W,D);
    ...
    b0ppmstep = stepppm/100;

    intfactor = 100;
    nzsp1 = 2*nppm-1;
    nzsp2 = intfactor*(nzsp1-1)+1;
    stepppm2 = b0ppmstep;
    rangezsp2 = (int)( 0.3/stepppm2 );
    ...
    // zsp1r  = mxCalloc( nzsp1, sizeof(double) );  /* raw z spectrum sorted */
    ...
    double *zsp1r  = (double *)malloc(nzsp1 * sizeof(double));  /* raw z spectrum sorted */
    ...
    for (ippm1 = 0; ippm1 < nppm; ippm1++)
    {
        ...
        // printf("index = %i psort = %i xzsp1 = %.4f    index = %i xzsp1 = %.4f\n",ippm1,psort[ippm1],xzsp[ippm1],ippm2,xzsp[ippm2]);
    }
    ...
    for (ielem = 0; ielem < nelem; ielem++)
    {
        ...
        if (mask[ielem] > 0)
        {
            ...
            /* Find zspectrum minimum position and scale zspectrum */
            temp1 = 1.0e38;
            temp2 = -1e38;
            for (ippm1 = 0; ippm1 < nzsp1; ippm1++)
            {
                ...
                if (zsp1[ippm1] > temp2)
                    temp2 = zsp1[ippm1];
                zsp1r[ippm1] = zsp1[nzsp1-1-ippm1];
            }

            /* Call spline to get second derivatives */
            spline(xzsp, zsp1r, nzsp1, zsp1r[0], zsp1r[nzsp1-1], y2);
            /* Call splint  for interpolations and find index for zsp2 minimum */
            for (i2 =0; i2< nzsp2; i2++)
            {
                splint(xzsp, zsp1r, y2, nzsp1, xzsp2[i2], &zsp2r[i2]);
            }
...
//            B0map[ielem] = rangezsp2;
//              B0map[ielem] = -rangezsp2;
            B0map[ielem] = -( temp1 + (minSSEindex * stepppm2) );
//              B0map[ielem] = temp1;
//              B0map[ielem] = (minSSEindex * stepppm2);
//                B0map[ielem] = i1;
        }
    ...
    // double *B0map =  (double *)malloc(960 * sizeof(double));
    // for (ielem = 0; ielem < 960; ielem++)
    // {
    //     B0map[ielem] = pimg[ielem];
    //   }
